...
export default {
    ...
    methods:{
        addTodo(){
            if(this.newTodoItem != ""){ //inputbox의 입력값이 있을 때만 저장
                var value = this.newTodoItem && this.newTodoItem.trim(); //inputbox에 입력된 텍스트의 앞뒤 공백 문자열 제거
                this.$emit('addTodo', value);
                this.clearInput(); //inputbox의 입력값을 초기화
            }
        },
        clearInput(){
            this.newTodoItem= '';
